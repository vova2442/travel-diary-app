<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    {{-- Эта ссылка будет видна всем --}}
                    <x-nav-link :href="route('community')" :active="request()->routeIs('community')">
                        {{ __('Сообщество') }}
                    </x-nav-link>

                    {{-- Эти ссылки будут видны ТОЛЬКО залогиненным пользователям --}}
                    @auth
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                @auth
                    {{-- Если пользователь залогинен, показываем выпадающее меню с именем --}}
                ...
                @else
                    {{-- Если пользователь - гость, показываем ссылки на вход и регистрацию --}}
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @auth
                <x-responsive-nav-link :href="route('travels.index')" :active="request()->routeIs('travels.index')">
                    {{ __('Мои путешествия') }}
                </x-responsive-nav-link>
            @endauth
        </div>
